...
function Messages() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-10">
      <h1 className="font-display text-3xl font-bold text-primary font-telugu mb-6">సందేశాలు</h1>
      ...
      {isLoading ? (
        ...
        <div className="royal-card p-12 text-center">
          ...
          <p className="font-telugu text-muted-foreground">ఇంకా సంభాషణలు లేవు</p>
          <Link to="/browse" className="btn-royal inline-block mt-5 px-6 py-2.5 rounded-full text-sm font-telugu">ప్రొఫైల్స్ చూడండి</Link>
        </div>
      ) : (
          ...
          {conversations.map((c: any) => (
            <li key={c.partner_id}>
              <Link to="/messages/$id" params={{ id: c.partner_id }} className="flex items-center gap-3 p-4 hover:bg-secondary/50">
                <div className="h-12 w-12 rounded-full btn-royal flex items-center justify-center font-bold font-telugu">{(c.profile?.full_name_telugu || c.profile?.full_name || "?").slice(0, 2)}</div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <div className="font-semibold font-telugu truncate">{c.profile?.full_name_telugu || c.profile?.full_name || "—"}</div>
